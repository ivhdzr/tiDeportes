        ...
        <div class="row">
            <div class="col-12">
                <a href="{{ route('torneos.index') }}" class="btn btn-light"><i class="nav-icon fas fa-arrow-circle-left"></i>
                    Regresar</a>
                <form autocomplete="off" action="{{ route('torneos.store') }}" method="post" class="mt-2">
                    @csrf
                    <div class="form-group">
                        <label for="nombre">Nombre</label>
                        <input autofocus required type="text" class="form-control" name="nombre" id="inputNombre"
                            placeholder="Ingresa el nombre del torneo" value="{{ old('nombre') }}">
                        @error('nombre')
                        ...
                        <div class="col-md-6 col-sm-12">
                            <div class="form-group">
                                ...
                                <input required type="datetime-local" class="form-control" name="fecha_inicio" id="inputFechaInicio"
                                    placeholder="Ingresa la fecha de incio" value="{{ old('fecha_inicio') }}">
                                @error('fecha_inicio')
                                    <div class="badge badge-danger text-wrap">
                                        {{ $message }}
                                    </div>
                        ...
                        <div class="col-md-6 col-sm-12">
                            <div class="form-group">
                                ...
                                <input required type="datetime-local" class="form-control" name="fecha_final" id="inputFechaFinal"
                                    placeholder="Ingresa la fecha de incio" value="{{ old('fecha_final') }}">
                                @error('fecha_final')
                                    <div class="badge badge-danger text-wrap">
                                        {{ $message }}
                                    </div>
                                ...
                                <select required name="sede_id" class="form-control">
                                    ...
                                    @foreach ($sedes as $sede)
                                        <option value="{{ $sede->id }}"
                                            {{ old('sede_id') == $sede->id ? 'selected' : '' }}>{{ $sede->nombre }}</option>
                                    @endforeach
                                </select>
