        ...
        <div class="row">
            <div class="col-12">
                <a href="{{ route('partidos.index') }}" class="btn btn-light"><i class="nav-icon fas fa-arrow-circle-left"></i>
                    Regresar</a>
                <form action="{{ route('partidos.update', $partido) }}" method="post" class="mt-2">
                    @csrf
                    @method('put')
                    <div class="form-group">
                        <label for="nombre">Nombre</label>
                        <input autofocus required type="text" class="form-control" name="nombre" id="inputNombre"
                            placeholder="Ingresa el nombre del partido" value="{{ old('nombre', $partido->nombre) }}">
                        @error('nombre')
                                ...
                                <select required name="torneo_id" class="form-control">
                                    ...
                                    @foreach ($torneos as $torneo)
                                        <option value="{{ $torneo->id }}"
                                            {{ old('torneo_id', $partido->torneo_id) == $torneo->id ? 'selected' : '' }}>
                                            {{ $torneo->nombre }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="col-md-6 col-sm-12">
                            <div class="form-group">
                                ...
                                <input required type="datetime-local" class="form-control" name="fecha_partido"
                                    ...
                                    value="{{ old('fecha_partido', $partido->fecha_partido) }}">
                                @error('fecha_partido')
                                    <div class="badge badge-danger text-wrap">
                                        {{ $message }}
                                    </div>
                                ...
                                <select required name="equipo_visitante_id" class="form-control">
                                    ...
                                    @foreach ($equipos as $equipo)
                                        <option value="{{ $equipo->id }}"
                                            ...
                                            {{ \Illuminate\Support\Facades\Crypt::decrypt($equipo->nombre) . ' (' . $equipo->deporte->name . ')' }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <select required name="arbitro_id" class="form-control">
                            ...
                            @foreach ($arbitros as $arbitro)
                                <option value="{{ $arbitro->id }}"
                                    {{ old('arbitro_id', $partido->arbitro_id) == $arbitro->id ? 'selected' : '' }}>
                                    {{ $arbitro->nombre }}
                                </option>
                            @endforeach
                        </select>
